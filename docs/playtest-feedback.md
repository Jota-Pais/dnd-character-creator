...
### F33 — Poderes Paranormais e aquisição de poder de outra classe não existem no sistema (Ordem)
- **Sistema:** Ordem Paranormal
- **Onde:** categoria inteira ausente — não há `data/paranormal-powers.json`, tipo, nem UI; `characterUtils.getAvailablePowerOptions` só oferece poderes da própria classe; `class-powers.json` tem `transcend` (Transcender) sem picker.
- **Relato:** ao gerar por importação uma ficha de Ocultista/Lâmina Paranormal (origem Acadêmico) para testar o app, três poderes descritos pelo usuário **não puderam ser representados** — porque são **Poderes Paranormais** (categoria à parte dos poderes de classe), que o sistema não modela: **Resistir a \<Elemento\>**, **Expansão de Conhecimento** e **Potencial Aprimorado**. Além disso, a build usava **Ninja Urbano** (poder de *especialista*) num ocultista — legal pelo livro **via Expansão de Conhecimento** (que concede um poder de classe de outra classe), mecânica que o app também não tem.
- **Triagem:** 📖 fidelidade ao livro + ✨ feature nova (categoria de regra inteira faltando + mecânica de aquisição cross-classe)
- **Confirmado no livro (texto digitalizado aqui pra implementação futura):**
  - **Resistir a \<Elemento\>** (poder paranormal): "Escolha entre Conhecimento, Energia, Morte ou Sangue. Você recebe resistência 10 contra esse elemento. Este poder conta como um poder do elemento escolhido. Afinidade: aumenta a resistência para 20."
  - **Expansão de Conhecimento** (poder de Conhecimento): "Você se conecta com o Conhecimento do Outro Lado, rompendo os limites de sua compreensão. Você aprende um poder de classe que não pertença à sua classe (caso o poder possua pré-requisitos, você precisa preenchê-los). Pré-requisito: Conhecimento 1. Afinidade: você aprende um segundo poder de classe que não pertença à sua classe."
  - **Potencial Aprimorado** (poder de Morte): "A Morte lhe concede potencial latente de momentos roubados de outro lugar. Você recebe +1 ponto de esforço por NEX. Quando sobe de NEX, os PE que recebe por este poder aumentam de acordo. (Ex.: escolhido em NEX 30% → 6 PE; ao subir para 35% → +1 PE, e assim por diante.) Afinidade: +1 PE adicional por NEX (total +2 PE por NEX)."
- **Confirmado no código:**
  1. **Não há categoria "Poderes Paranormais":** nenhum `paranormal-powers.json`, nenhum tipo, nenhuma tela. O único vestígio é o texto do poder de classe `transcend` ("Escolha um poder paranormal…").
  2. **Transcender é beco sem saída:** `transcend` está em `class-powers.json` (as 3 classes) mas fora de `POWER_PARAM_SPECS` → o `ProgressionStep` não mostra picker; escolher Transcender grava só `powerChoices[i]="transcend"`, sem onde registrar *qual* poder paranormal foi ganho (nem seus efeitos como +Afinidade).
  3. **Sem aquisição cross-classe:** `getAvailablePowerOptions` → `getPowersByClass(cls.id)` filtra estritamente pela classe do agente; não existe caminho pra um ocultista pegar `urban-ninja` (especialista). Não dá pra representar o efeito de Expansão de Conhecimento.
  4. **Outros pontos de entrada de poder paranormal também sem UI:** origem **Cultista Arrependido** (`repentant-cultist`) — "Traços do Outro Lado: Você possui um poder paranormal à sua escolha" — não tem seletor (só o texto do poder de origem).
- **Menções anteriores (dispersas, sempre adiadas — este item consolida):** F24 ("Transcender sem picker"), F25 (pendências: Transcender), F27 ("Transcender — precisa digitalizar os poderes paranormais do livro, sessão dedicada"), F31 (pendências justificadas: "Traços do Outro Lado / Transcender — Poderes Paranormais ainda não digitalizados"). **Novo aqui:** o caso completo da categoria + a **aquisição de poder de outra classe** (Expansão de Conhecimento), que nunca foi registrada.
- **Causa-raiz (por que passou):** o `ROADMAP.md` (Fase 9, recorte do livro) marcou o Cap. 5 "O Outro Lado" como `✅ parcial — NEX, poderes e rituais`, com a linha listando "Poderes Paranormais" — mas o que foi entregue foi **poder de CLASSE (Cap. 1) + rituais (Cap. 5)**; os **Poderes Paranormais (Cap. 5) nunca foram digitalizados**. O checkmark conflou as duas coisas e mascarou o gap. Nenhum layer (docs/dados/tipos/UI) tem a categoria.
- **A decidir na execução:** (1) digitalizar a lista de Poderes Paranormais do livro (com regra de **Afinidade** por elemento) num `paranormal-powers.json` + tipo; (2) modelar como o agente adquire um poder paranormal (Transcender, Cultista Arrependido, e o "poder paranormal à escolha" em geral) — provavelmente um campo novo no draft + picker no passo Progressão/Origem; (3) modelar a **aquisição cross-classe** de Expansão de Conhecimento (permitir escolher um poder de outra classe, respeitando pré-requisitos) — impacta `getAvailablePowerOptions` e a validação; (4) aplicar efeitos determinísticos na ficha (Resistir a Elemento → resistência; Potencial Aprimorado → +PE por NEX retroativo, como o F16/F31).
- **Impacto no teste que originou o item:** a ficha de importação do "Ceifeiro" teve que **substituir** esses poderes pelos ocultistas mais próximos (Especialista/Mestre em Elemento em Conhecimento, Ritual Potente) e **não conseguiu** representar Ninja Urbano no ocultista — a identidade de combate ficou só na trilha Lâmina Paranormal.
- **Implementado (2026-07-17), em 9 commits:**
  1. **Docs:** `docs/ordem-paranormal/regras-poderes-paranormais.md` — Transcender, Afinidade Elemental, elementos opressores, limite de rituais aprendidos e os 22 poderes com textos integrais (Sangue de Ferro remontado do OCR embaralhado). Inclui as decisões de interpretação confirmadas com o usuário (linha "Afinidade" exige a 2ª escolha; afinidade ativa com transcender de aquisição NEX ≥ 50; contagem cronológica de "Elemento N"; SAN do Cultista = metade só da inicial, ex. ocultista NEX 5% → 15).
  2. **Dados/tipos:** `data/paranormal-powers.json` (22 poderes com `effects`/`affinityEffects` como deltas) + `types/paranormalPower.ts` + `types/prereq.ts` (pré-requisito estruturado compartilhado); os 18 poderes de classe com pré-requisito textual ganharam `prereqs` validáveis; origem Cultista marcada com `grantsParanormalPower`/`halvesStartingSanity`.
  3. **Motor:** `paranormalPowerUtils.ts` — passo único cronológico (origem → NEX 15/30/45 → Versatilidade → 60/75/90) que valida pré-requisitos com motivo legível, detecta a 2ª cópia por afinidade, aplica círculo/limite (= Intelecto) do Aprender Ritual e a **Expansão de Conhecimento cross-classe** (valida os pré-requisitos do alvo; o poder aprendido acende a maquinaria existente via `hasClassPower`), com invalidação em cascata.
  4. **Efeitos na ficha de verdade:** PV/PE retroativos por NEX (Sangue de Ferro +20 PV em NEX 50%, Potencial Aprimorado +6 PE em NEX 30% — números do livro em teste), Defesa (Precognição), resistências por elemento (Resistir 10/20), perícias (+5), limite de PE (Encarar a Morte), margem de ameaça/multiplicador (Golpe de Sorte), rituais do Aprender Ritual na ficha, e a Sanidade: cada Transcender suprime o ganho de 1 NEX e o Cultista corta metade da SAN inicial (clamp ≥ 0).
  5. **UI:** nova etapa "Poderes Paranormais" (entre Progressão e Rituais) com catálogo por elemento, opções bloqueadas sempre visíveis com o motivo, seção de Afinidade Elemental, sub-escolhas aninhadas (ritual/elemento/poder de outra classe) e integrações (aviso do Cultista na Origem, status do Transcender na Progressão, rituais concedidos pra qualquer classe no passo Rituais); Revisão e ficha imprimível com seção própria, breakdown de SAN e números do motor.
  6. **Fix adjacente:** trocar o poder de um slot (ou da Versatilidade) agora descarta os `powerParams` órfãos da instância (antes o passo Progressão travava pra sempre).
- **Status:** ✅ FEITO (2026-07-17) — 65 testes novos do motor + integridade de dados (774 no total), verificado no browser (Playwright/Edge, 29 checks): retrocompat de ficha antiga, catálogo com bloqueio por pré-requisito, cascata de invalidação, SAN do cultista (25→15), resist 20 com 2ª escolha, ritual concedido pra Combatente e mobile 390px.
