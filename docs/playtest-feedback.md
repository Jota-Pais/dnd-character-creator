...
- **Pendências restantes do pacote:** Transcender (precisa digitalizar os poderes paranormais do livro), Mochila de Utilidades (escolher item — interage com categoria/carga), Especialista em Elemento não anota o +2 de DT na ficha (a DT exibida é a geral), e o parâmetro de poder escolhido via **Versatilidade** não tem UI (o modelo já suporta a chave `versatility`).
- **Status:** ✅ FEITO (2026-07-12) — +3 testes (577 no total).
