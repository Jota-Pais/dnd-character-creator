...
| 2026-07-31 | **Modo de jogo (ficha viva)** aberto como frente nova, isolada do fluxo de criação. `IRpgSystem.play?` é opcional — sistema sem adaptador não é jogável. A ficha é **imutável durante o jogo**: sessão em chave própria (`play-sessions`), com teste de browser conferindo que o draft não muda |
| 2026-07-31 | Motor de dados (`src/core/dice`) puro, semeável e **separado da apresentação** — requisito de produto: a versão final terá animação de rolagem, e ela entra como camada de UI sem tocar no cálculo    |
| 2026-07-31 | Dano modelado com **dados separados do bônus** (`DamageSpec`) desde o início, porque o crítico do Ordem multiplica só os dados (p. 84) — string tipo `"1d8+2"` não serve                              |
